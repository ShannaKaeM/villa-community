...
class Main implements IntegrationInterface
{
    public function __construct()
    {
        ...
        if ($this->is_enabled()) {
            ...
            new \WindPress\WindPress\Integration\Builderius\Editor();
        }
    }
    ...
    {
        return (bool) \apply_filters('f!windpress/integration/builderius:enabled', Config::get(\sprintf('integration.%s.enabled', $this->get_name()), \true));
    }
    public function register_provider(array $providers) : array
    {
        $providers[] = ['id' => $this->get_name(), 'name' => \__('Builderius', 'windpress'), 'description' => \__('Builderius integration', 'windpress'), 'callback' => \WindPress\WindPress\Integration\Builderius\Compile::class, 'enabled' => $this->is_enabled()];
        return $providers;
    }
