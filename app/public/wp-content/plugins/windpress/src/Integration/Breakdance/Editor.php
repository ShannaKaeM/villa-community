...
class Editor
{
    public function __construct()
    {
         ...
         */
        \add_action('unofficial_i_am_kevin_geary_master_of_all_things_css_and_html', fn() => $this->editor_assets(), 1000001);
    }
    public function editor_assets()
    {
        ...
        \add_filter('f!windpress/core/runtime:append_header.ubiquitous_panel.is_prevent_load', static fn($is_prevent_load) => \true);
        Runtime::get_instance()->append_header();
        \wp_head();
        $handle = WIND_PRESS::WP_OPTION . ':integration-breakdance-editor';
        AssetVite::get_instance()->enqueue_asset('assets/integration/breakdance/main.js', ['handle' => $handle, 'in_footer' => \true]);
        \wp_localize_script($handle, 'windpressbreakdance', ['_version' => WIND_PRESS::VERSION, 'assets' => ['url' => AssetVite::asset_base_url()], 'site_meta' => ['name' => \get_bloginfo('name'), 'site_url' => \get_site_url(), 'admin_url' => AdminPage::get_page_url()]]);
